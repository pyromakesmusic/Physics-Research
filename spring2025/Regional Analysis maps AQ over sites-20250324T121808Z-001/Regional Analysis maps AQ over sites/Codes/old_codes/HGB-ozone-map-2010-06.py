...
# Define key sites to highlight with full labels
key_sites = [8, 553, 557, 561, 698]  # Most important monitoring sites
...
for idx, row in ozone_data.iterrows():
    ...
    is_key_site = cams_num in key_sites
    
    # Different positioning strategy for key vs. regular sites
    if is_key_site:
        # For key sites, use more prominent labels with larger font
        # Calculate angle for better distribution
        angle = (cams_num % 8) * 45  # 0, 45, 90, 135, 180, 225, 270, 315 degrees
        site_distance = distance * 1.2  # Slightly further for key sites
