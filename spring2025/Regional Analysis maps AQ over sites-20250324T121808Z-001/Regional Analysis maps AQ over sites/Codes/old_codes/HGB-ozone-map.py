# -*- coding: utf-8 -*-
"""
HGB Region Daily Ozone Map
Fixed CSV reading to handle the specific format of your data files
"""
...
# Parameters
target_day = 21  # Change this to visualize different days of the month
month_name = "June"
year = 2010
...
try:
    ...
    print(f"TCEQ columns: {dataTCEQsites.columns.tolist()}")
...
ozone_file_path = "G:/.shortcut-targets-by-id/1WLOv_L9wwz6Q8Xn55EiDYIyuYgN2nuta/AQ  Ozone and PM for HGB SOMs/Regional Analysis maps AQ over sites/Data/MDA8_O3/HGB_MDA8-O3_201006.txt"
...
# The ozone data file uses "Monitoring_Site" for site names
site_column = None
...
xticks_major = np.arange(-96, -93.95, 0.5)
xticks_minor = np.arange(-96, -93.95, 0.1)
yticks_major = np.arange(29, 30.34, 0.5)
yticks_minor = np.arange(29, 30.34, 0.1)
...
cmap = plt.cm.RdYlBu_r  # Use RdYlBu_r for ozone data
...
# Plot ozone values with LARGE squares for better visibility
scatter = ax1.scatter(
    ozone_data['CAMS_Long'], 
    ozone_data['CAMS_Lat'],
    c=ozone_data['ozone'],
    s=map_point_size * 1.2,  # Much larger for visibility
    cmap=cmap,
    norm=norm,
...
if not exceed_sites.empty:
    ax1.scatter(
        ...
        s=map_point_size * 1.5,  # Larger highlight
        facecolors='none',
        edgecolors='red',
        linewidths=line_width_value * 1.5,  # Thicker outline
        marker='o',
        zorder=11
    )

# Modify these size parameters for better label positioning
label_font_size = 10  # Smaller font for less clutter
label_pad = 1.0       # Smaller padding in the text boxes
box_alpha = 0.8       # Transparency of label boxes
...
for idx, row in ozone_data.iterrows():
    ...
    # Advanced positioning system for labels to reduce overlap
    # Calculate angle based on CAMS number to position labels in different directions
    angle = (cams_num % 8) * 45  # 0, 45, 90, 135, 180, 225, 270, 315 degrees
    distance = 0.08  # Base distance from point
    
    # Calculate offset using trigonometry for more distributed label positions
    ...
    # Add label with ozone value - SMALLER, CLEANER FONT
    ax1.text(
        ...
        bbox=dict(
            facecolor='white', 
            alpha=box_alpha, 
            edgecolor=text_color if row['ozone'] > 0.070 else 'gray',
            pad=label_pad, 
...
# Add a legend to explain the CAMS numbers
from matplotlib.patches import Patch

# Create a list of principal monitoring sites to highlight in the legend
important_sites = [
    {'cams': 1, 'name': 'Houston East'},
    {'cams': 8, 'name': 'Houston Aldine'},
    {'cams': 35, 'name': 'Deer Park'},
    {'cams': 78, 'name': 'Conroe'},
    {'cams': 698, 'name': 'Jones Forest'},
]
...
# Set title with date and exceedance info
title = f'Ozone Concentrations on {month_name} {target_day}, {year}\n' \
